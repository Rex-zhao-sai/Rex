...
export function PhotoUploader({
    ...
    async (type: "before" | "after", file: File) => {
      ...
      try {
        ...
        try {
          ...
          console.warn("[PhotoUpload] Supabase upload failed, using base64 fallback:", supabaseError);
          // 降级到 base64
          s3Key = null;
        }
        ...
        const photoRecord: PhotoRecord = {
          ...
          s3Key: s3Key || undefined, // 用于持久化存储（Supabase key），base64 回退时为 undefined
  ...
  // 压缩图片为 Blob（最大 1280px，质量 0.7，如果压缩后更大则保留原图）
  const compressImage = (file: File, maxWidth: number = 1280, quality: number = 0.7): Promise<Blob> => {
